import torch
from utils.evaluation_v8 import validate
...
def train(model,
    ...
    for epoch in range(start_epoch, num_epochs):
        ...
        for batch_idx, batch in enumerate(progress):
            ...
            target_edit_ops = target_edit_ops.to(device, non_blocking=True)
            target_lengths = target_lengths.to(device, non_blocking=True)

            with torch.amp.autocast(device_type='cuda', enabled=True):
                te = target_edit_ops.clone()
                B, T = te.size()
                te = torch.where(te < 0, torch.full_like(te, ignore_id), te)
                pos = torch.arange(T, device=te.device).unsqueeze(0).expand(B, -1)
                mask_beyond = pos >= target_lengths.unsqueeze(1)
                te = torch.where(mask_beyond, torch.full_like(te, ignore_id), te)

                logits, targets_out = model(
                    audio_emb=audio_emb,
                    text_emb=text_emb,
                    feats=feats,
                    target_edit_ops=te
                ...
                with torch.no_grad():
                    pos_shift = torch.arange(Tm1, device=target_lengths.device).unsqueeze(0).expand(Bm, -1)
                    valid_mask = pos_shift < torch.clamp(target_lengths - 1, min=0).unsqueeze(1)
